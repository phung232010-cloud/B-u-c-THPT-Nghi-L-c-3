...
import { BarChart3, Users, Vote, TrendingUp, Trophy, Activity } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const statCards = [
    { icon: Users, label: 'Tổng ứng viên', value: candidates.length, change: '+2', color: 'text-primary' },
    { icon: Vote, label: 'Tổng phiếu bầu', value: totalVotes, change: '+12%', color: 'text-accent' },
    { icon: Trophy, label: 'Ứng viên dẫn đầu', value: sorted[0]?.name.split(' ').pop() || '-', change: `${sorted[0]?.voteCount} phiếu`, color: 'text-success' },
    { icon: Activity, label: 'Tỷ lệ tham gia', value: '68%', change: '+5%', color: 'text-warning' },
  ];
    ...
    <AppLayout>
      <div className="p-6 md:p-8 max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-muted-foreground mb-8">Tổng quan hệ thống bỏ phiếu real-time</p>
        </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass rounded-2xl p-6">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={barData}>
                ...
                <Tooltip
                  contentStyle={{ background: 'hsl(222, 47%, 9%)', border: '1px solid hsl(222, 30%, 18%)', borderRadius: '12px', color: 'hsl(210, 40%, 96%)' }}
                />
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="font-semibold">Xu hướng bỏ phiếu theo giờ</h3>
          </div>
        ...
        {/* AI Prediction */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="glass rounded-2xl p-6 mt-6 gradient-hero">
          <div className="flex items-center gap-2 mb-4">
            <TrendingUp className="w-5 h-5 text-accent" />
            <h3 className="font-semibold">AI Prediction</h3>
            <span className="ml-auto text-xs px-2 py-0.5 rounded-full bg-accent/20 text-accent">Live</span>
          </div>
          <div className="grid md:grid-cols-3 gap-4">
            {sorted.slice(0, 3).map((c, i) => {
              const confidence = [87.3, 72.1, 58.5][i];
              const trend = ['↑ Tăng mạnh', '→ Ổn định', '↓ Giảm nhẹ'][i];
              return (
                <div key={c.id} className="glass rounded-xl p-4">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-lg gradient-primary flex items-center justify-center font-bold text-primary-foreground">
                      {i + 1}
                    </div>
                    <div>
                  ...
                  <div className="text-2xl font-bold gradient-text">{confidence}%</div>
